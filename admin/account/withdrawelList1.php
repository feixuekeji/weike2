	...
	<center>
        ...
        <tr>
			...
			while ($rowUser=mysql_fetch_assoc($rstUser)){
				echo "<tr>";
				echo "<td>{$rowUser['withdrawelID']}</td>";
				echo "<td>{$rowUser['accID']}</td>";
				echo "<td>{$rowUser['withdrawelAmount']}</td>";
				echo "<td>".date('Y年m月d日 H:i:s',$rowUser['withdrawelSubtime'])."</td>";
				echo "<td>{$rowUser['withdrawelStatus']}</td>";
				echo "<td><a href='checkWithdrawel.php?id={$rowUser['withdrawelID']}&accID={$rowUser['accID']}&amount={$rowUser['withdrawelAmount']}'>审核</a></td>";
				echo "</tr>";
			}
			?>
