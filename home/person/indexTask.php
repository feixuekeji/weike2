...
$sqlUser="select * from task where UserID={$id} order by ID desc limit {$off},{$length}";
	...
	<div class="tab-con cjhz rcb xsgl" style="height:90%;">
		...
		<div class="con">
			...
			while ($rowUser=mysql_fetch_assoc($rstUser)){
				...
				echo "<li>";
				echo "<p>";
				echo "<span class='col-220'><a href='../task/task.php?id={$rowUser['ID']}'>{$rowUser['Name']}</a></span>";
				...
				echo "<span class='col-12'><a href='../task/task.php?id={$rowUser['ID']}' target='_blank'> 查看</a> ｜ <a href='delete.php?id={$rowUser['ID']}'>删除</a></span>";
				echo "</p>";
				echo "</li>";
				echo "</ul>";
			}
