...
//                }
        ...
        close: function ($img) {
            ...
            }).parent().find('.edui-image-close').on("click",function () {
		...
		insert: function (editor,$img,url,_url) {

            $img.parent().find('.edui-image-insert').on("click",function () {
				editor.execCommand('insertimage', {src: url,_src:_url});
            });

            return this;
		},
        callback: function (editor, $w, url, state,id,_url) {
			var lang = editor.getLang('image');
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                var $img = $("<img src='" + editor.options.imagePath + url + "' alt='" + editor.options.imagePath + _url + "' class='edui-image-pic' />"),
                    $item = $("<div class='edui-image-item edui-image-upload-item' id='"+id+"'><div class='fa fa-file-text edui-image-insert' title='"+lang.btnInsert+"'></div><div class='fa fa-trash-o edui-image-close' title='"+lang.btnDelete+"'></div></div>").append($img);
                ...
                $img.on("load", function () {
                    Base.scale(this, 120);
					Base.insert(editor,$(this),editor.options.imagePath+url,editor.options.imagePath+_url);
                    Base.close($(this));
    ...
    var Upload = {
        ...
        undefinedimage: function () {
			var me = this;
			$.getJSON(upload_undefinedimage, function(json){
				if(json.status=='ok'){
					if(json.data[0].fid){
						$(".edui-image-upload2", this.dialog).show();
            			$(".edui-image-dragTip", this.dialog).hide();
           				$(".edui-image-upload1", this.dialog).hide();
					}
					$.each(json.data, function(key,file) {
						 Base.callback(me.editor, me.dialog,file.thumb,'SUCCESS',file.fid,file.filepath);
					});
				}
			});
        },
        ...
        config: function (sel) {
            ...
            $("form", $(sel, me.dialog)).attr("action", url).append($('input[name="'+TokenName+'"]').clone(true));
        ...
        uploadComplete: function(r){
            ...
            try{
                var json = eval('('+r+')');
                Base.callback(me.editor, me.dialog, json.url, json.state, json.id,json._url);
            }catch (e){
            ...
            $(".edui-image-searchAdd", me.dialog).on("click", function () {
                ...
                if (url) {
                    ...
                    $("<img src='" + url + "' class='edui-image-pic' />").on("load", function () {
                        ...
                        var $item = $("<div class='edui-image-item'><div class='fa fa-file-text edui-image-insert' title='"+lang.btnInsert+"'></div><div class='fa fa-trash-o edui-image-close' title='"+lang.btnDelete+"'></div></div>").append(this);
						...
						Base.insert(me.editor,$(this),url);
    ...
    UM.registerWidget('image', {
        ...
        width: 700,
        height: 408
    }, function (editor, $w, url, state , id, _url) {
        Base.callback(editor, $w, url, state ,id ,_url)
    })
})();
