...
$sqlUser="select * from withdrawel,account where withdrawel.accID = account.accID and withdrawel.accID={$accID}  order by withdrawelID limit {$off},{$length}";
	...
	<div class="tab-con cjhz rcb xsgl" style="height:90%;">
		...
		<div class="con">
			<ul class="head">
				<li>
					<p>
						<span class="col-200">账号</span>
						<span class="col-200">提现金额</span>
						<span class="col-200">提现时间</span>
						<span class="col-200">状态</span>
				</li>
			</ul>
			<?php
			while ($rowUser=mysql_fetch_assoc($rstUser)){
				...
				echo "<li>";
				echo "<p>";
				echo "<span class='col-200'>{$rowUser['accName']}</span>";
				echo "<span class='col-200'>{$rowUser['withdrawelAmount']}</span>";
				echo "<span class='col-200'>".date('Y-m-d H:i',$rowUser['withdrawelSubtime'])."</span>";
				switch($rowUser['withdrawelStatus'])
				{
				case 0:
				echo "<span class='col-200'>未审核</span>";
				break;
				case 1:
				echo "<span class='col-200'>通过</span>";
				break;
				case 2:
				echo "<span class='col-200'>未通过</span>";
				break;
				}
