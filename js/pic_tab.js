...
jq(".radio-btn").on('click', function () {
    var _this = jq(this),
...
jq.fn.banqh = function(can){
	...
	jq(can.psnum).find('ul').width(picminnum*picminw).height(picminnum*picminh);
...
//点击小图切换大图
		...
		//点击小图切换大图
	    jq(can.psnum).find('li').click(function () {
        tpqhnum = xtqhnum = jq(can.psnum).find('li').index(this);
        show(tpqhnum);
		minshow(xtqhnum);
    }).eq(can.order).trigger("click");
//大图弹出框
...
//大图切换过程
		function show(tpqhnum){
			...
			jq(can.psnum).find('li').eq(tpqhnum).addClass("on").siblings(this).removeClass("on");
		};
//弹出框图片切换过程
