...
//获取数据列数，实现翻页
$sqlCnt="select count(*) from withdrawel";
...
$sqlUser="select * from withdrawel,account where withdrawel.accID = account.accID order by withdrawelID limit {$off},{$length}";
	...
	<div class="tab-con cjhz rcb xsgl" style="height:90%;">
		...
		<div class="con">
			<ul class="head">
				<li>
					<p>
						...
						<span class="col-150">提现金额</span>
						<span class="col-180">提现时间</span>
			...
			while ($rowUser=mysql_fetch_assoc($rstUser)){
				...
				echo "<li>";
				echo "<p>";
				echo "<span class='col-60'>{$rowUser['withdrawelID']}</span>";
				echo "<span class='col-100'>{$rowUser['accName']}</span>";
				echo "<span class='col-150'>{$rowUser['withdrawelAmount']}</span>";
				echo "<span class='col-180'>".date('Y-m-d H:i',$rowUser['withdrawelSubtime'])."</span>";
				switch($rowUser['withdrawelStatus'])
				{
				case 0:
				...
				if ($rowUser['withdrawelStatus']==0) {
					echo "<span class='col-12'><a href='checkWithdrawel.php?id={$rowUser['withdrawelID']}&accID={$rowUser['accID']}&amount={$rowUser['withdrawelAmount']}'>审核通过</a></span>";
				}else
